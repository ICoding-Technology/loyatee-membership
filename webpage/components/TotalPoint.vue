        ...
        <div>
            ...
            <div class="flex items-baseline justify-between">
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-400">{{ total - current }} more points to reach Gold</p>
                </div>
                <div class="text-right">
                    <p class="text-lg font-semibold text-[#3B82F6]">{{ current.toLocaleString() }}</p>
            ...
            <div class="mt-4 h-1.5 w-full rounded-full bg-slate-200 overflow-hidden">
                <div class="h-full rounded-full bg-[#3B82F6] transition-all duration-300"
                    :style="{ width: computedPercent + '%' }"></div>
            </div>
        </div>
    </div>
</template>
